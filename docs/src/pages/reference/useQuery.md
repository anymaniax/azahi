...
```ts
const queryResult = this.azahi.useQuery(queryKey, queryFn?, {
  cacheTime,
  enabled,
  initialData,
...
const result = this.azahi.useQuery({
  queryKey,
  queryFn,
  enabled,
})
...
- `queryFn: (context: QueryFunctionContext) => Promise<TData> | Observable<TData>`
